function TDCdemo
    ...
    NET.addAssembly('C:/Program Files (x86)/Signadyne/Libraries/VisualStudio/Reference Assemblies/VS2012/Signadyne.dll');

    slot = 2;
    part = 'SD-PXE-TDC-H0002';

    % Create module object
    tdc = Signadyne.SD_TDC();
    ...
    tdc.DAQconfig(nChannel, points2read, 1, 1, Signadyne.SD_TriggerModes.VIHVITRIG);
    ...
    while nPointsRead < points2read
        ...
        if error < 0
            display(['error : ', char(Signadyne.SD_Error.getErrorMessage(error))]);
        end;
    end;
