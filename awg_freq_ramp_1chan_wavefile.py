"""
Single channel, single waveform frequency ramp from a waveform file.

P. Huft

Connect a 3.3 V trigger to the AWG card and run this script to generate a 
Gaussian frequency ramp once per trigger. The wave will be sinusoidal
when not ramping.

Note that the output veiwed on a scope may look to bulge in amplitude
where the ramp happens. Zooming in reveals that this is a sampling error.
"""
...
if error < 0:
    ...
    error = card.modulationAngleConfig(
        nChannel=0,
        modulationType=1, # 0: off, 1: frequency, 2: phase
        deviationGain=1e5
        )
